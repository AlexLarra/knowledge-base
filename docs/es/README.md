# Guías para nuevos programadores

Bienvenido. Este sitio recoge guías prácticas y explicaciones paso a paso pensadas para quienes están dando sus primeros pasos en el mundo de la programación.

El objetivo es ofrecer recursos sencillos, directos y fáciles de entender. Cada guía parte de cero, sin asumir conocimientos previos, para que puedas aprender con tranquilidad y a tu propio ritmo.

## Guías disponibles

- [git: Por qué usar Git](git/why-use-git.md)
- [git: Git para niños (solo CLI)](git/git-for-kids-cli.md)
- [git: De Git local a GitHub](git/from-local-git-to-github.md)
- [git: diff-so-fancy (opcional)](git/diff-so-fancy.md)
- [tmux: Por qué usar tmux](tmux/why-use-tmux.md)
- [tmux: Crear un workspace con tmux](tmux/tmux-workspace.md)
- [tmux: Tmux para scripts en el servidor](tmux/use-tmux-for-production-scripts.md)
